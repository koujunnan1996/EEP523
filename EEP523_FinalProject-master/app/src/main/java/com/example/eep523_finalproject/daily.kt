...
class daily : AppCompatActivity(), BottomNavigationView.OnNavigationItemSelectedListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        running2.setOnClickListener {
            // your code to perform when the user clicks on the button
            if(running2.getText().toString() == "Running") {
                Toast.makeText(this@daily, "Running added.", Toast.LENGTH_SHORT).show()
                running2.setText("Running*");
                running2.setBackgroundColor(R.color.icon_navi);
                map.put("Running",false);
            }else  if(running2.getText().toString() == "Running*") {
                Toast.makeText(this@daily, "Running removed.", Toast.LENGTH_SHORT).show()
                running2.setText("Running");
                running2.setBackgroundColor(Color.WHITE);
                map.remove("Running");
            }
        }
