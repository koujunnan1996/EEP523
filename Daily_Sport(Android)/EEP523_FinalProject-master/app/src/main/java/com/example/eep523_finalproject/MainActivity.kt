...
class MainActivity : AppCompatActivity(), BottomNavigationView.OnNavigationItemSelectedListener,
    ...
    fun upDateButtons(){
        for ((key, value) in map) {
            ...
            linearLayout.addView(button)
            if(tasks == map.size){
                val textView = findViewById(R.id.title) as TextView
                textView.setText("Congrads, you finished all tasks today!")
                textView.setBackgroundColor(R.color.c1)
            }
        }
        if(map.size== 8 ){
            val linearLayout = findViewById(R.id.fragment_container) as LinearLayout
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            ...
            R.id.Kou -> {
                val intent = Intent(this, BLEConnection::class.java)
                intent.putExtra("map", map);
                startActivity(intent)
                return true
            }
