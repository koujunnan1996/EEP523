...
import kotlinx.android.synthetic.main.activity_pullup.*
import java.util.HashMap


class pullup : AppCompatActivity(), SensorEventListener {
...
//    private val SHAKE_SLOP_TIME_MS = 500
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val exit = findViewById(R.id.pullup_exist) as Button
        exit.setOnClickListener {
            val map = intent.getSerializableExtra("map") as HashMap<String, Boolean>
            if(isFinished==true){
                map.put("Pull-Up",true)
            }
            val intent = Intent(this, MainActivity::class.java)
            intent.putExtra("where", "pullup");
...
//        val az = event.values[2]
        ...
        if (work) {
            if (event.values[1] >= 10f) {
                val now = System.currentTimeMillis()
                if (now - shaketime < 2000) {
                    return
                }
                shaketime = now
