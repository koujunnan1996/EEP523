...
class pushup : AppCompatActivity(), SensorEventListener {
...
//    private val SHAKE_SLOP_TIME_MS = 500
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val exit = findViewById(R.id.pushup_exist) as Button
        exit.setOnClickListener {
            val map = intent.getSerializableExtra("map") as HashMap<String, Boolean>
            if(isFinished==true){
                map.put("Push-Up",true)
            }
            val intent = Intent(this, MainActivity::class.java)
            intent.putExtra("where", "pushup");
...
//        val az = event.values[2]
        ...
        if (work) {
            if (event.values[1] >= 12f) {
                val now = System.currentTimeMillis()
                if (now - shaketime < 1000) {
                    return
                }
                shaketime = now
